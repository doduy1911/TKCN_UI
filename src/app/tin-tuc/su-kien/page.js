...
const Page = async ({ searchParams }) => {
   ...
   const sukien = await useData("posts", 3, currentPage);

   return (
      <div className="container pt-6">
         <DetailHeader dataDetail={{ name: "Sự kiện" }} />
         <section className="mx-auto flex justify-between gap-[60px] py-8 max-md:flex-col">
            <div className="flex flex-1 flex-col gap-[30px]">
               {sukien?.data?.map((item, index) => (
                  <Link
                     prefetch={false}
                     href={`/${item?.slug}`}
               ...
               {sukien?.meta?.last_page > 1 && <Pagination dataPage={sukien} />}
            </div>
