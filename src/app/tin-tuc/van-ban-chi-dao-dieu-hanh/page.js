...
const Page = async ({ searchParams }) => {
   ...
   const dataVBCDDH = await useData("posts", 8, currentPage);

   return (
      <div className="container pt-6">
         <DetailHeader dataDetail={{ name: "Văn bản chỉ đạo điều hành" }} />
         <section className="mx-auto flex justify-between gap-[60px] py-8 max-md:flex-col">
            <div className="flex flex-1 flex-col gap-[30px]">
               {dataVBCDDH?.data?.map((post, index) => (
                  <Link
                     prefetch={false}
                     href={`/${post?.slug}`}
                  ...
                  >
                     ...
                     <div className="flex-1">
                        <h5 className="text-[1.15em] font-bold">{post?.name}</h5>
               ...
               {dataVBCDDH?.meta?.last_page > 1 && (
                  <Pagination dataPage={dataVBCDDH} />
               )}
            </div>
